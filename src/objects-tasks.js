...
function mergeObjects(objects) {
  ...
  objects.forEach((obj) => {
    ...
    objEntries.forEach((item) => {
      newObj[item[0]] =
        item[0] in newObj !== true ? item[1] : (newObj[item[0]] += item[1]);
    });
  });
  return newObj;
}
...
function compareObjects(obj1, obj2) {
  ...
  return obj1Keys.every(
    (elem) => obj2Keys.includes(elem) && obj1[elem] === obj2[elem]
  );
}
...
function makeWord(lettersObject) {
  const arr = [];
  arr.length = Object.values(lettersObject).flat().length;
  const objEntries = Object.entries(lettersObject);
  objEntries.forEach((arrayItem) => {
    const [currentValue, currentPos] = arrayItem;
    currentPos.forEach((elem) => {
      arr[elem] = currentValue;
    });
  });
  return arr.join('');
}
...
function sortCitiesArray(arr) {
  ...
  function compareObj(objA, objB) {
    ...
    if (objA.country === objB.country) {
      if (objA.city > objB.city) return 1;
      if (objA.city < objB.city) return -1;
    }
    return 0;
  }
  arrCopy.sort(compareObj);
  return arrCopy;
}
...
function group(array, keySelector, valueSelector) {
  const resultMap = array.reduce((accum, item) => {
    if (accum.has(keySelector(item)) !== true) {
      accum.set(keySelector(item), [valueSelector(item)]);
    } else {
      accum.get(keySelector(item)).push(valueSelector(item));
    }
    return accum;
  }, new Map());
  return resultMap;
